...
def get_bigquery_schema(dataset_id, client=None, project_id=None, allowed_tables=None):
    """Retrieves schema and generates DDL for allowed tables only.

    Args:
        dataset_id (str): The ID of the BigQuery dataset.
        client (bigquery.Client): A BigQuery client.
        project_id (str): The ID of your Google Cloud Project.
        allowed_tables (list): Optional list of table IDs to include.
    ...
    for table in client.list_tables(dataset_ref):
        # Filtering: Skip tables not in the allowed list
        if allowed_tables and table.table_id not in allowed_tables:
            continue
        ...
        rows = list(client.list_rows(table_ref, max_results=5))
        if rows:
            ddl_statement += f"-- Example values for table `{table_ref}`:\n"
            for row in rows:  # Iterate over Row objects
                ddl_statement += f"INSERT INTO `{table_ref}` VALUES\n"
                example_row_str = "("
                for value in row.values():  # Row values
...
def run_bigquery_validation(
    ...
    if match := re.search(
        ...
        final_result["error_message"] = (
            f"Invalid SQL: Contains disallowed DML/DDL operations. Found forbidden keyword: '{match.group(1)}'"
        )
        return final_result
